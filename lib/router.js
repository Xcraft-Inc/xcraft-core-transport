...
class Router {
  ...
  keepAlive() {
    if (!this._mode.startsWith('push')) {
      throw new Error(
        `keep-alive is only possible with push mode (current is ${this._mode})`
      );
    }

    const backend = this._backends.get('axon');
    if (!backend) {
      return;
    }

    backend.send(':keep-alive');
  }
