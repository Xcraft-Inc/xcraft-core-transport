...
class Router {
    ...
    if (arpEntries.length === 0) {
      ...
      if (lineId) {
        if (lines[lineId]) {
          ...
          for (const _orcName in lines[lineId]) {
            for (const backend in arp) {
    ...
    if (arpEntries.length > 0) {
      ...
      for (const {orcName, entry, router, forwarding} of arpEntries) {
        ...
        if (
          ...
          throw new Error(
            'Routed to abyss, line used with not router defined ?!'
          );
        }
      }
    }
  ...
  static deleteRoute(orcName, backend) {
    ...
    if (backend === 'axon') {
      ...
      for (const lineId in lines) {
        delete lines[lineId][orcName];
      }
    }
  }

  static updateLines(type, lineId, orcName, token) {
    const _token = require('xcraft-core-bus').getToken();
    if (token === _token) {
      return;
    }
    ...
    switch (type) {
      ...
      case 'delete':
        ...
        delete lines[lineId][orcName];
        if (Object.keys(lines[lineId]).length === 0) {
          delete lines[lineId];
        }
        break;
    }
  ...
  static connectLine(lineId, orcName) {
    ...
    if (!lines[lineId][orcName] || lines[lineId][orcName] === -1) {
      lines[lineId][orcName] = 1;
    } else {
      ++lines[lineId][orcName];
      return;
    }
  ...
  static disconnectLine(lineId, orcName) {
    if (!lines[lineId] || !lines[lineId][orcName]) {
      //throw new Error('try to disconnect an unmanaged line');
      return;
    }

    --lines[lineId][orcName];
    if (lines[lineId][orcName] === 0) {
      delete lines[lineId][orcName];
    } else {
      return;
    }
