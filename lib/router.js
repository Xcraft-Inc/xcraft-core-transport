...
class Router {
  ...
  on(topic, handler) {
    ...
    this._backends.forEach(backend => backend.on(topic, keys, handler));
    return this;
  }
  ...
  subscribe(topic) {
    ...
    if (!(topic instanceof RegExp)) {
      const stripped = xUtils.regex.stripXcraftRegExpStr(topic);
      const _keys = stripped.replace(/\\/g, '').split(/\n|::/);
      for (const key of _keys) {
        if (!key.length) {
          break;
        }
    ...
    this._backends.forEach(backend => backend.subscribe(regTopic, keys));
  }
