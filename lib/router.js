...
class Router {
  ...
  _pub(fromLines, counter, topic, ...args) {
    ...
    if (
      (msg && msg.forwarding && msg.forwarding.appId) || // Forward cast for an Xcraft node (appId)
  ...
  static _requestLineUpdate(lineId, orcName, type, xcraftRPC) {
    ++lines.generation;
    ...
    resp.command.nestedSend('warehouse.request-line-update', payload, (err) => {
      if (payload._xcraftRPC) {
        resp.log.warn(err.stack || err.message || err);
      } else {
  ...
  static connectLine(lineId, orcName, xcraftRPC = false) {
    /* local */
    if (orcName === 'greathall') {
    ...
    /* remote */
    Router._requestLineUpdate(lineId, orcName, 'add', xcraftRPC);
  }

  static disconnectLine(lineId, orcName, xcraftRPC = false) {
    /* local */
    if (orcName === 'greathall') {
      const p = [lineId, orcName];
      const cnt = lines.local.getIn(p, 0);
    ...
    /* remote */
    Router._requestLineUpdate(lineId, orcName, 'delete', xcraftRPC);
  }
  ...
  subscribe(topic, backend, orcName, xcraftRPC = false) {
    if (this._mode !== 'sub') {
      throw new Error(
    ...
    if (orcName) {
      ...
      if (lineId) {
        Router.connectLine(lineId, orcName, xcraftRPC);
      }
  ...
  unsubscribe(topic, backend, orcName, xcraftRPC = false) {
    if (this._mode !== 'sub') {
      throw new Error(
    ...
    if (orcName) {
      const lineId = extractLineId(topic);
      if (lineId) {
        Router.disconnectLine(lineId, orcName, xcraftRPC);
      }
    }
