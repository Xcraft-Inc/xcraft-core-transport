...
const lines = {
  ...
  remote: new fromJS({}) /* The remote (our own remote lines too) lines */,
...
class Router {
  ...
  static _requestLineUpdate(lineId, orcName, type) {
    ...
    resp.command.nestedSend(
      ...
      {type, lineId, orcName, token, generation: lines.generation},
      (err) => {
        if (err) {
