...
class Router {
  constructor(registry, id, mode, log) {
  ...
  send(topic, ...args) {
    ...
    /* Routing */
    const byOrcName = topic.endsWith('.finished') || topic.endsWith('.error');

    if (byOrcName) {
      const orcName = topic.substr(0, topic.indexOf('::'));
      if (arp[orcName]) {
        const {id, socket} = arp[orcName];
        if (id !== this._id) {
          const router = this._routers[id].pub;
        ...
        /* Use this instance */
        const msgId =
          (args && args[0] && args[0]._xcraftMessage && args[0].id) || '';
        const isNested = msgId.endsWith('-nested');
        const backendName =
          !isNested && socket instanceof net.Socket ? 'axon' : 'ee';
  ...
  _insertRoute(id, orcName, token, socket) {
