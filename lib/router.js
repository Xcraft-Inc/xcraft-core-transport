...
class Router {
    ...
    this._backends.forEach((backend, name) =>
      this._probe(`${name}/${this._mode}/${topic}`, null, () =>
        backend.on(topic, (...args) => {
          const _1 = args[0];
          const _2 = args[1];
          /* Keep an ARP table of which server is associated to which orcName */
          if (this.mode === 'pull' && topic === 'message' && _2.arp) {
            const socket = args[2];
            // XXX: check if already exists in the ARP table
            Object.keys(_2.arp).forEach(orcName =>
              Router.insertRoute(orcName, _2.arp[orcName], socket)
            );
          } else if (topic === 'close') {
            const socket = args[2];
            Router.deleteRoute(socket);
          }
          handler(_1, _2);
        })
      )
  ...
  static insertRoute(orcName, token, socket) {
    // arp[orcName] = {token, socket};
  }

  static deleteRoute(socket) {}
  ...
  connect(backend, options, callback) {
    ...
    this._backends.get(backend).connect(
      options,
      callback
    );
  }
