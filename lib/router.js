...
class Router {
      ...
      this._backends.forEach((backend) =>
        backend
          ...
          .on(
            ...
            (err, sock) => {
              if (sock) {
                Router._destroyEvtSocket(backend, sock);
              }
      ...
      this._backends.forEach((backend) =>
        backend
          .on('error', (err, sock) => {
            const remotePort = sock.remotePort;
            if (remotePort) {
              Router._destroyCmdSocket(backend, remotePort);
            }
          })
          .on('disconnect', (sock) => {
            const remotePort = sock.remotePort;
  ...
  static _destroyCmdSocket(backend, evtRemotePort) {
  ...
  static _destroyEvtSocket(backend, cmdSocket) {
    const arpEntry = Object.entries(arp[backend.name]).find(
     ...
     */
    const [, port] = arpEntry;
    this._backends.get(backend).destroySockets([port]);
  }
