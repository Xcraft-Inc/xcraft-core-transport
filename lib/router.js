...
class Router {
        ...
        backend
          ...
          .on(
            ...
            (sock) => {
              ...
              this._log.dbg(
                `command bus client disconnected, remote port: ${remotePort}`
              );
              this._destroyEvtSocket(backend, sock);
      ...
      this._backends.forEach((backend) =>
        backend
          ...
          .on('disconnect', (sock) => {
            const remotePort = sock.remotePort;
            this._log.dbg(
              `event bus client disconnected, remote port: ${remotePort}`
            );
            this._destroyCmdSocket(backend, remotePort);
          })
      );
  ...
  static connectLine(lineId, orcName, routerId, _backup = false) {
    let exit = true;
    let lines = {};
    ...
    if (_backup) {
      backupLines.set(`${orcName}::${lineId}`, true);
    }
  ...
  static disconnectLine(lineId, orcName, routerId, _backup = false) {
    let exit = true;
    ...
    if (_backup) {
      backupLines.delete(`${orcName}::${lineId}`);
    }
  ...
  static resendLines() {
    ...
    backupLines.forEach((router, key) => {
      const [orcName, ...lineId] = key.split('::');
      list.push({orcName, lineId: lineId.join('..')});
    });
    ...
    Object.values(routers)
      ...
      .map((router) => router.push)
      .forEach((router) =>
        list.forEach(({orcName, lineId}) =>
          router.send(`:conn-line`, {
            lineId,
            orcName,
            routerId: router.id,
          })
  ...
  subscribe(topic, backend, orcName) {
    ...
    if (orcName) {
      ...
      if (lineId) {
        ...
        Router.connectLine(lineId, orcName, id, true);
      }
  ...
  unsubscribe(topic, backend, orcName) {
    ...
    if (orcName) {
      ...
      if (lineId) {
        const {id} = routers[this._id].push;
        Router.disconnectLine(lineId, orcName, id, true);
      }
    }
  }
