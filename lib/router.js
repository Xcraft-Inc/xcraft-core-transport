...
class Router {
    ...
    if (mode === 'pull') {
      ...
      this._backends.forEach((backend) =>
        backend
          ...
          .on('socket need drain', (sock) => {
            const {remoteAddress, remotePort, writableLength} = sock;
            /* FIXME: impl. for the streams (pause if writableLength is too high) */
            if (!remotePort || writableLength < 100_000_000) {
              return;
            }

            this._log.dbg(
              `event bus socket ${remoteAddress}:${remotePort} need drain for ${writableLength} bytes`
            );
            sock.destroySoon();
            this._destroyCmdSocket(backend, remotePort);
          })
  ...
  static #getBus() {
    if (!Router.#xBus) {
      Router.#xBus = require('xcraft-core-bus');
    }
    return Router.#xBus;
  }

  static #getBusClient() {
    if (!Router.#xBusClient) {
      Router.#xBusClient = require('xcraft-core-busclient');
    }
    return Router.#xBusClient;
  }

  static #getHost() {
    if (!Router.#xHost) {
      Router.#xHost = require('xcraft-core-host');
    }
    return Router.#xHost;
  }
  ...
  static #getHorde() {
    if (!Router.#xEtc) {
      Router.#xEtc = require('xcraft-core-etc')();
    }
    if (Router.#xEtc && !Router.#xHorde) {
      Router.#xHorde = require('xcraft-core-horde');
    }
    return Router.#xHorde;
  }
  ...
  _mapEntry(orcName, backend) {
    const {appMasterId} = Router.#getHost();
        ...
        if (useDefaultGateway) {
          ...
          for (const orcName of Object.keys(arp.axon)) {
            if (arp.axon[orcName].nodeName === appMasterId) {
              arpEntries.push(this._mapEntry(orcName, 'axon'));
          ...
          for (const entry of __lines.get(lineId).keys()) {
            ...
            for (const backend in arp) {
              const entries = Object.keys(arp[backend])
                ...
                .map((orcName) => this._mapEntry(orcName, backend));
  ...
  static updateLines(_lines, _token, generation, horde) {
    ...
    const token = Router.#getBus().getToken();
    if (token !== _token) {
      return;
    }
  ...
  static _requestLineUpdate(lineId, orcName, type) {
    ...
    const token = Router.#getBus().getToken();
    const busClient = Router.#getBusClient().getGlobal();
  ...
  subscribe(topic, backend, orcName) {
    ...
    const str = toXcraftRegExpStr(topic);
  ...
  *_start(options, next) {
  ...
  static _toRegExp(str) {
    return str instanceof RegExp ? str : new RegExp(toXcraftRegExpStr(str));
  }
}
