...
class Router {
  constructor(mode, log) {
    ...
    this._isProbing = xProbe.isAvailable();
  ...
  on(topic, handler) {
    this._backends.forEach(backend =>
      this._probe(`${this._mode}/${topic}`, null, () =>
        backend.on(topic, handler)
      )
    );
    return this;
  }
  ...
  send(topic, ...args) {
    ...
    let transports = [];
    if (args && args[0] && args[0]._xcraftMessage) {
      transports = args[0]._xcraftTransports;
    }
    ...
    if (this._connectedWith) {
      ...
      this._probe(`${this._mode}/${topic}`, args, () =>
    ...
    this._backends.forEach((backend, name) => {
      ...
      return this._probe(`${this._mode}/${topic}`, args, () =>
