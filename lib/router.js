...
class Router {
            ...
            if (args[0].startsWith(':')) {
              switch (args[0]) {
                case ':delete-route':
                  ...
                  break;
                case ':conn-line':
                  Router.connectLine(args[1].lineId, args[1].orcName);
                  break;
                case ':disconn-line':
                  Router.disconnectLine(args[1].lineId, args[1].orcName);
                  break;
              }
              return;
            }
  ...
  send(topic, ...args) {
    ...
    if (topic.endsWith('.finished') || topic.endsWith('.error')) {
      if (!msg.forwarding) {
        ...
        const entry = Object.entries(arp).find(
          ([, entry]) =>
    ...
    if (arpEntries.length === 0) {
      ...
      if (lineId) {
        arpEntries = Object.keys(arp)
          .filter(
            (orcName) => !!arp[orcName].lines && !!arp[orcName].lines[lineId]
          )
          .map((orcName) => {
            return {
              orcName,
    ...
    if (arpEntries.length > 0) {
      ...
      for (const {orcName, entry} of arpEntries) {
        const {id, port} = entry;
        if (id !== this._id) {
    ...
    /* else, continue with broadcasting */
  ...
  _insertRoute(id, orcName, entry, socket) {
    const {token, port, hordes} = entry;
    arp[orcName] = merge(arp[orcName] || {}, {
      id,
      token,
      socket,
      port,
  ...
  static connectLine(lineId, orcName) {
    ...
    if (!arp[orcName].lines) {
      arp[orcName].lines = {};
    }

    if (arp[orcName].lines[lineId]) {
      return;
    }

    arp[orcName].lines[lineId] = true;

    Object.values(routers)
      .filter((router) => router.push && router.push.connectedWith() === 'axon')
      .map((router) => router.push)
      .forEach((router) => {
        router.send(`:conn-line`, {lineId, orcName});
      });
  }

  static disconnectLine(lineId, orcName) {
    if (!arp[orcName] || !arp[orcName].lines) {
      return;
    }
    ...
    delete arp[orcName].lines[lineId];

    Object.values(routers)
      .filter((router) => router.push && router.push.connectedWith() === 'axon')
      .map((router) => router.push)
      .forEach((router) => {
        router.send(`:disconn-line`, {lineId, orcName});
      });
  }
  ...
  static moveRoute(oldOrcName, newOrcName) {
    arp[newOrcName] = arp[oldOrcName];
    if (!arp[newOrcName].token) {
      arp[newOrcName].token = newOrcName.split('@')[1];
    }
    Router.deleteRoute(oldOrcName);
  }
  ...
  subscribe(topic, backend, orcName) {
    ...
    if (orcName) {
      const lineId = extractLineId(topic);
      if (lineId) {
        const router = routers[this._id].push;
        router.send(':conn-line', {lineId, orcName});
      }
    }
