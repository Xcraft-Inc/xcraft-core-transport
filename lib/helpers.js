...
class Helpers {
  ...
  static tryStreamTo(msg, newStreamer) {
    ...
    } else if (msg.data.xcraftUpload) {
      const {xcraftUpload} = msg.data;
      if (Array.isArray(xcraftUpload)) {
        const pack = tar.pack();
        let i = 0;
        for (const file of xcraftUpload) {
          fs.createReadStream(file).pipe(
            pack.entry({name: path.basename(file)}, () => {
              if (i === xcraftUpload.length - 1) {
                pack.finalize();
              }
            })
          );
        ...
        stream = pack;
        msg._xcraftStreamTar = true;
      } else if (typeof xcraftUpload === 'string') {
        stream = fs.createReadStream(xcraftUpload);
      } else {
