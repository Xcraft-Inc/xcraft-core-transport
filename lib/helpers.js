...
class Helpers {
  ...
  static _tryStreamTo(msg, stream, newStreamer) {
  ...
  static toXcraftJSON(args, newStreamer = null) {
    ...
    return args.map(d => {
      ...
      if (d.data instanceof Readable) {
        Helpers._tryStreamTo(d, d.data, newStreamer);
      } else if (d.data.xcraftStream) {
        Helpers._tryStreamTo(d, d.data.xcraftStream, newStreamer);
      }
