...
class EE {
  ...
  sendTo(port, topic, streamChannel, msg, ...args) {
    ...
    if (msg) {
      ...
      helpers.tryStreamTo(msg, (streamId, stream) => {
        const info = streamChannel({stream});
        Object.assign(msg.data.xcraftStream, info);
        return info;
      });
    }
  ...
  static _toRegExp(str) {
    const {toAxonRegExpStr} = require('xcraft-core-utils/lib/regex.js');
    return str instanceof RegExp ? str : new RegExp(toAxonRegExpStr(str));
  }
}
