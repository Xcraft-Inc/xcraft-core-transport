...
function bufferFromData(dataUrl) {
  return dataUrl.startsWith('base64:')
    ...
    : fse.readFileSync(dataUrl);
}

// Very simple test for localhost
function isLocalhost(address) {
  return /^(localhost|127\.0\.0\.1|::1|0\.0\.0\.0)$/i.test(address);
}
...
class Axon {
  ...
  get isLocalOnly() {
    return this.isUnixSocket || isLocalhost(this._host);
  }
  ...
  sendTo(port, topic, streamChannel, ...args) {
    ...
    const getData = () =>
      helpers.toXcraftJSON(args, (streamId, stream, isUpload) =>
  ...
  start(options, callback) {
    ...
    if (options.keyPath && options.certPath) {
      ...
      this._tlsOptions = {
        ...
        key: fse.readFileSync(options.keyPath),
        cert: fse.readFileSync(options.certPath),
