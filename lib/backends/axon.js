...
class Axon {
  ...
  on(topic, handler) {
    this._sock.on(topic, (...args) => {
      const data = args.map(d => {
        ...
        if (d._xcraftShredder) {
          const n = Object.assign({}, d);
          n.data = new Shredder(transit.fromJSON(d.data));
          delete n._xcraftImmutable;
          return n;
        }
  ...
  send(topic, ...args) {
    ...
    const data = args.map(d => {
      ...
      if (d.data._isSuperReaper6000) {
        const n = Object.assign({}, d);
        n.data = transit.toJSON(d.data.state);
        n._xcraftShredder = true;
        return n;
      }
    });
