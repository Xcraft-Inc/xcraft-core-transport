...
class Axon {
  ...
  on(topic, handler) {
    const h = (...args) => {
      if (topic === 'message' && this._sock.socks.length === 0) {
        return;
      }

      let data = args;
      if (topic === 'message') {
        data = helpers.fromXcraftJSON(args);
        /* Keep an ARP table of which server is associated to which orcName */
        if (this._mode === 'pull' && data[1].arp) {
          // XXX: check if already exists in the ARP table
          Object.keys(data[1].arp).forEach(orcName =>
            Router.updateARP(orcName, data[1].arp[orcName])
          );
        }
      }
  ...
  send(topic, ...args) {
    ...
    const data = helpers.toXcraftJSON(args);
